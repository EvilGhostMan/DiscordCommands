...
const defaultConfig = require(path.join(__dirname, "../package.json"))
  .defaultConfig;
...
const template = fs
  .readFileSync(
    path.join(
      __dirname,
      args[0] == "remote" ? "template.remote.js" : "template.local.js"
    )
  )
  .toString();
...
for (let f = 0; f < list.length; f++) {
  ...
  const minified = UglifyJS.minify("module.exports = " + content);

  let result = formatString(template, {
    PLUGIN_NAME: pluginName,
    CONFIG: JSON.stringify(config),
    INNER: minified.code.replace(/module\.exports\s*=\s*/, ""),
