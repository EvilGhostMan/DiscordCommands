...
const list = args.slice(1).length
  ...
  : fs
      ...
      .filter(
        (f) =>
          fs.lstatSync(path.join(pluginsPath, f)).isDirectory() &&
          f != "0PluginLibrary"
      );
(async (list) => {
  console.log("");
  ...
  for (let f = 0; f < list.length; f++) {
    ...
    const configPath = isLibrary
      ? path.join(__dirname, "..", "src", "config.js")
      : path.join(pluginsPath, output, "config.json");
    ...
    const config = defaults({
      ...
      PLUGIN_PATH: path.join(configPath, "..", pluginConfig.main),
    });
...
`;
    ...
    let result = fs
      .readFileSync(path.join(config.output.path, config.output.filename))
      .toString();
    ...
    if (libConfig.copyToBD) {
      ...
      fs.writeFileSync(
        path.join(bdFolder, "plugins", config.output.filename),
        result
      );
    }
